...
- PR `#1` (`feat/m1-schema-freeze` -> `main`) is still `OPEN` / `CLEAN` on published head `1de61fa` with `13` unresolved non-outdated review threads; refresh GitHub before using this note as merge evidence because the thread count is volatile.
