...
Current status:
- Milestone 1 is still carried by PR #1 (`feat/m1-schema-freeze`), which remains `OPEN` / `CLEAN` on published head `dc4dd88`.
...
- GitHub now shows 7 unresolved non-outdated review threads, and the latest bounded local review again timed out before a terminal verdict, so the branch is not locally review-cleared.
